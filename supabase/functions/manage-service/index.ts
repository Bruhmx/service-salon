...
Deno.serve(async (req) => {
  ...
  try {
    ...
    console.log(`Service management action: ${action} by user ${user.id}`);

    if (action === 'create' || action === 'update') {
      const validationResult = serviceSchema.safeParse(body.data);
      ...
      console.log(`Service deleted: ${validationResult.data.id}`);
